...
public class CommandSetTempBlock extends CommandBase {
    ...
    public void onExecute(CommandSender sender, String label, String[] args) throws Exception {
        if (args.length == 7) {
            if (Bukkit.getWorld(args[0]) != null && MapUtils.isNumber(args[1]) && MapUtils.isNumber(args[2]) && MapUtils.isNumber(args[3]) && Material.valueOf(args[4]) != null && MapUtils.isNumber(args[5])) {
                ...
                }.runTaskLater(MapUtils.getPlugin(), Integer.parseInt(args[5]) * 20);
                return;
            }
        }
